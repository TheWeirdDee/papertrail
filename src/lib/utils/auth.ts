...
// WARNING: This only decodes the token — signature verification must happen server-side.
...
export const getTokenExpirationTime = (token: string): number => {
  try {
    ...
    const secondsUntilExpiry = (decoded.exp as number) - Math.floor(Date.now() / 1000);
...
export const getAddressFromToken = (token: string): string | null => {
  try {
    const decoded = decodeToken(token);
    return (decoded?.address as string) || null;
  } catch {
    return null;
  }
};
